static char help[] = "Euler implicit \n\n";
...
/* HDF5: Initialization */
  ...
  //set boundary conditions
   VecSetValue(uold,0,0.0,INSERT_VALUES);// BC: U0=0;
   VecSetValue(uold,n,0.0,INSERT_VALUES);// BC: Un=0;
   ierr = VecAssemblyBegin(uold);CHKERRQ(ierr);
   ierr = VecAssemblyEnd(uold);CHKERRQ(ierr);
 ...
 ierr = VecView(f,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr); 
  ...
  {
    rstart = 1;
    i      = 0; col[0] = 0; col[1] = 1; value[0] = 1; value[1] = 0;  // BC
    ierr   = MatSetValues(A,1,&i,2,col,value,INSERT_VALUES);CHKERRQ(ierr);
  }
  if (rend == nn) 
  {
    rend = nn-1;
    i    = nn-1; col[0] = nn-2; col[1] = nn-1; value[0] = 0; value[1] = 1;// BC
    ierr = MatSetValues(A,1,&i,2,col,value,INSERT_VALUES);CHKERRQ(ierr);
  }
...
bool  restart = false;
  ...
  while(error>tol){
	  error = 0.0;
	  ierr = VecAYPX(uold,1,f);CHKERRQ(ierr);
//ierr = VecView(uold,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);  
...
//ierr = VecView(u,PETSC_VIEWER_STDOUT_WORLD);CHKERRQ(ierr);
	  ...
	  error=PetscAbsReal((normU-normUold));
